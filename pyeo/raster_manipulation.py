...
def apply_scl_cloud_mask(
    ...
    log.info(f"{len(safe_file_path_list)} L2A raster files marked for SCL cloud masking.")
    for l2_safe_file in safe_file_path_list:
...
def apply_scl_cloud_mask_to_filelist(
    ...
    for l2_safe_file in safe_file_path_list:
        nfile = nfile+1
        log.info("---------------------------------------------------")
        log.info(f"Processing L2A raster file {nfile} out of {nfiles}")
...
def apply_processing_baseline_offset_correction_to_tiff_file_directory(
    ...
    '''
    #TODO: generated by ChatGPT, this could be a solution to get band-specific offset values:
    # Read individual BOA_ADD_OFFSET value for each band from xml information in SAFE file root
    # Work out why offset files are larger (2GB from ~1GB)
    
    import xml.etree.ElementTree as ET
    
    # Path to the Sentinel-2 L2A metadata file
    metadata_file = 'path/to/your/MTD_MSIL2A.xml'
    
    # Parse the XML file
    tree = ET.parse(metadata_file)
    root = tree.getroot()
    
    # Define the XML namespace (ns)
    ns = {'n1': 'https://psd-14.sentinel2.eo.esa.int/level-2a/standard/1.1'}
    
    # Find all band-specific BOA_ADD_OFFSET elements
    boa_add_offset_elements = root.findall('.//n1:BOA_ADD_OFFSET_List/n1:BOA_ADD_OFFSET', ns)
    
    # Initialize a dictionary to store the offsets by band
    boa_add_offsets = {}
    
    # Loop through the offset elements and extract the band-specific values
    for offset_element in boa_add_offset_elements:
        # Extract band ID and offset value
        band_id = offset_element.get('band_id')
        offset_value = float(offset_element.text)
        
        # Store the band-specific offset in the dictionary
        boa_add_offsets[band_id] = offset_value
    
    # Display the BOA_ADD_OFFSET values for each band
    for band_id, offset_value in boa_add_offsets.items():
        print(f"Band {band_id}: BOA_ADD_OFFSET = {offset_value}")
    '''
    ...
    for f in image_files:
        ...
        if get_processing_baseline(f)[0] != "A" and \
            ...
            in_raster_path = os.path.join(in_tif_directory, f)
            log.info(f"Offsetting file: {f}")
            #log.info(f'TMP: Full file path: {in_raster_path}')
            ...
            #log.info(f"TMP: out_temporary_raster_path: {out_temporary_raster_path}")
            ...
            #log.info(f"TMP: in_raster_array dtype: {in_raster_array.dtype}")
            dtype_max = 10000
            log.info(f"Pixel values clipped to range of min: {-1 * BOA_ADD_OFFSET} and max: {dtype_max} then offset by: {BOA_ADD_OFFSET}")
            ...
            #   algorithm can run
            #log.info(f"TMP: Moving {out_temporary_raster_path} to {out_raster_path}")
            shutil.move(out_temporary_raster_path, out_raster_path)
            ...
            #log.info(f"TMP: Moving {out_raster_path} to {out_raster_path_rename}")
            shutil.move(out_raster_path, out_raster_path_rename)
...
def roi_tile_intersection(
    ...
    #log.info(f"ROI filepath: {roi_filepath}")
    ...
    tilelist_filepath = os.path.join(config_dict["tile_dir"], "tilelist.csv")
